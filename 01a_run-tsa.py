...
def run_tsa():
    ...
    sns.violinplot(y=stratum_col, x='SITE_INDEX', data=f_.reset_index(), ax=ax2, bw=bw, order=labels, linewidth=linewidth, inner=inner, width=width, cut=cut)
        ...
        for i, (si_level, Q) in enumerate(si_levelquants.items()):
            ...
            for j, species in enumerate(sv.index.values):
                ...
                try:
                    ...
                    print('fit error', exc, 'stratum', sc, 'si', si_level, 'species', species, 'n', len(x))
                    continue
    ...
    def run_vdyp(s, vdyp_ply, vdyp_lyr, vdyp_io_dirname='vdyp_io', 
                 ...
                 nsamples='auto', vdyp_binpath='VDYP7/VDYP7/VDYP7Console.exe',
        ...
        if shutil.which('wine') is None:
            raise RuntimeError('wine not found; VDYP7 requires wine to run on non-Windows systems')
    ...
    elif (not os.path.isfile(vdyp_results_tsa_pickle_path)) and os.path.isfile(vdyp_results_pickle_path):
        try:
