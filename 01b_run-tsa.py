...
def run_tsa(
    *,
    ...
    managed_curve_x_scale = float(
        os.environ.get("FEMIC_MANAGED_CURVE_X_SCALE", "0.8")
    )
    managed_curve_y_scale = float(
        os.environ.get("FEMIC_MANAGED_CURVE_Y_SCALE", "1.2")
    )
    managed_curve_max_age = int(os.environ.get("FEMIC_MANAGED_CURVE_MAX_AGE", "300"))
    managed_curve_truncate_at_culm = (
        os.environ.get("FEMIC_MANAGED_CURVE_TRUNCATE_AT_CULM", "1")
        .strip()
        .lower()
        in {"1", "true", "yes"}
    )
    ...
    if tipsy_input_df.empty:
        ...
        # reformat data
        for i in range(1, 6):
            tipsy_input_df[["PCT_" + str(i)]] = tipsy_input_df[["PCT_" + str(i)]].fillna(0)
            if tipsy_input_df["PCT_" + str(i)].dtype == object:
                tipsy_input_df["PCT_" + str(i)] = pd.to_numeric(
                    tipsy_input_df["PCT_" + str(i)]
                ).astype(int)
            else:
                tipsy_input_df["PCT_" + str(i)] = tipsy_input_df["PCT_" + str(i)].astype(int)
        ...
        if not Path(tipsyout).is_file():
            ...
            dp = tipsy_input_df.groupby(["AU", "TBLno"], as_index=False)[["Proportion"]].sum()
